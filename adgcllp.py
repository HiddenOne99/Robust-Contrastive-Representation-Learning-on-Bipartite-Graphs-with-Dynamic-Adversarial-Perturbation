...
from sklearn.linear_model import LogisticRegression # Added for link prediction evaluation
from sklearn.metrics import roc_auc_score, average_precision_score # Added for link prediction evaluation
...
def load_interactions(file_path, sep_pattern=r"\s+", header=None,
                      names=['user','item','label'], has_prefix=True):
    """Load interactions from a file into a pandas DataFrame."""
    df = pd.read_csv(
...
def build_mappings_from_train(train_df):
    ...
    users = sorted(positive_train_df['user'].unique())
    items = sorted(positive_train_df['item'].unique())
...
def build_joint_adjacency_and_normalize(train_df, user_map, item_map, num_users, num_items):
    ...
    positive_train_df = train_df[train_df['label'] == 1]
    df_filtered = positive_train_df[positive_train_df['user'].isin(user_map) & positive_train_df['item'].isin(item_map)]
    ...
    # The shape of the full matrix is (num_users + num_items) x (num_users + num_items)
    adj_rows_r = rows_r
    ...
    all_data = np.concatenate([data_r, data_r]) # Data is symmetric too
...
class LightGCNModel(Model):
    ...
    def call(self, adj_tf, training=False):
        ...
        for l in range(1, self.num_layers):
            ...
            # Store and apply noise for view 1
...
# Evaluation
def evaluate_model(gnn_model, train_df, user_map, item_map, test_df, reverse_user_map, reverse_item_map, num_users, num_items, train_adj_coo):
    ...
    test_users_orig = test_df['user'].values
    test_items_orig = test_df['item'].values
    test_labels_orig = test_df['label'].values

    all_test_users = np.array([user_map.get(u, -1) for u in test_users_orig])
    all_test_items = np.array([item_map.get(i, -1) for i in test_items_orig])
    all_test_labels = test_labels_orig

    valid_indices = (all_test_users != -1) & (all_test_items != -1)
    all_test_users = all_test_users[valid_indices]
    all_test_items = all_test_items[valid_indices]
    all_test_labels = all_test_labels[valid_indices]

    test_user_embs = tf.gather(user_embs_clean_online, all_test_users)
    test_item_embs = tf.gather(item_embs_clean_online, all_test_items)

    test_pairs_concatenated = tf.concat([test_user_embs, test_item_embs], axis=1)

    X_test = gnn_model.scorer_dense1(test_pairs_concatenated).numpy()
    
    positive_train_df_for_lr = train_df[train_df['label'] == 1]
    lr_train_pos_users_orig = positive_train_df_for_lr['user'].values
    lr_train_pos_items_orig = positive_train_df_for_lr['item'].values

    lr_train_pos_users = np.array([user_map.get(u, -1) for u in lr_train_pos_users_orig])
    lr_train_pos_items = np.array([item_map.get(i, -1) for i in lr_train_pos_items_orig])

    valid_lr_pos_indices = (lr_train_pos_users != -1) & (lr_train_pos_items != -1)
    lr_train_pos_users = lr_train_pos_users[valid_lr_pos_indices]
    lr_train_pos_items = lr_train_pos_items[valid_lr_pos_indices]

    num_lr_train_pos = len(lr_train_pos_users)
    lr_train_neg_users, lr_train_neg_items = [], []
    
    train_adj_csr = train_adj_coo.tocsr()
    user_pos_items_train = {u: train_adj_csr[u].indices for u in range(num_users)}

    lr_sampled_neg_count = 0
    while lr_sampled_neg_count < num_lr_train_pos:
        u = random.randrange(num_users)
        i = random.randrange(num_items)
        if (u not in user_pos_items_train or i not in user_pos_items_train[u]): # Ensure negatives are not in training positives
            lr_train_neg_users.append(u)
            lr_train_neg_items.append(i)
            lr_sampled_neg_count += 1

    all_lr_train_users = np.concatenate([lr_train_pos_users, lr_train_neg_users])
    all_lr_train_items = np.concatenate([lr_train_pos_items, lr_train_neg_items])
    all_lr_train_labels = np.concatenate([np.ones(num_lr_train_pos), np.zeros(num_lr_train_pos)])

    lr_train_user_embs = tf.gather(user_embs_clean_online, all_lr_train_users)
    lr_train_item_embs = tf.gather(item_embs_clean_online, all_lr_train_items)

    lr_train_pairs_concatenated = tf.concat([lr_train_user_embs, lr_train_item_embs], axis=1)

    X_train_lr = gnn_model.scorer_dense1(lr_train_pairs_concatenated).numpy()
    
    lg = LogisticRegression(penalty='l2', C=0.001, max_iter=500)
    lg.fit(X_train_lr, all_lr_train_labels)

    lg_y_pred_est = lg.predict_proba(X_test)[:, 1]

    auc_roc = roc_auc_score(all_test_labels, lg_y_pred_est)
    auc_pr = average_precision_score(all_test_labels, lg_y_pred_est)

    print(f"Link Prediction Metrics: AUC_ROC = {auc_roc:.6f}, AUC_PR = {auc_pr:.6f}")
    
    final_results = {'auc_roc': auc_roc, 'auc_pr': auc_pr}
    per_user_metrics = {}
...
# Main Training Loop
def train_model(joint_adj_tf, train_df, test_df, gnn_model, epsilon_generator, hparams, reverse_user_map, reverse_item_map, num_users, num_items, user_map, item_map):
    ...
    positive_train_df = train_df[train_df['label'] == 1]
    train_adj_csr = coo_matrix((np.ones(len(positive_train_df), dtype=np.float32), 
                                                (positive_train_df['user'].map(user_map).values, positive_train_df['item'].map(item_map).values)),
                                               shape=(num_users, num_items)).tocsr()
    user_pos_items = {u: train_adj_csr[u].indices for u in range(num_users)}
    
    users_all, items_all = train_adj_csr.nonzero()
    
    best_auc_roc_so_far = 0.0
    ...
    def train_step(u_batch_indices, v_batch_indices, v_neg_rank_indices, batch_idx, current_lambda_cl):
        ...
        with tf.GradientTape() as gen_tape:
            ...
            learned_dist = tfp.distributions.Normal(loc=gnn_model.generated_epsilon_means, 
                                                    scale=tf.exp(gnn_model.generated_epsilon_log_stds))
            # Gen KL
            L_kl_gen = tf.reduce_sum(tfp.distributions.kl_divergence(learned_dist, prior_dist))
        ...
        with tf.GradientTape() as model_tape:
            ...
            pos_user_embs = tf.gather(batch_user_clean_online, tf.range(tf.shape(u_batch_indices)[0])) 
            pos_item_embs = tf.gather(batch_item_clean_online, tf.range(tf.shape(v_batch_indices)[0])) 

            pos_pairs_concatenated = tf.concat([pos_user_embs, pos_item_embs], axis=1)

            pos_scores = tf.squeeze(gnn_model.scorer_dense2(gnn_model.scorer_dense1(pos_pairs_concatenated)), axis=-1)
            
            neg_user_embs = tf.gather(batch_user_clean_online, tf.range(tf.shape(u_batch_indices)[0]))
            neg_item_embs = tf.gather(batch_item_clean_online, tf.range(tf.shape(v_batch_indices)[0], tf.shape(all_batch_item_ids)[0]))

            neg_pairs_concatenated = tf.concat([neg_user_embs, neg_item_embs], axis=1)

            neg_scores = tf.squeeze(gnn_model.scorer_dense2(gnn_model.scorer_dense1(neg_pairs_concatenated)), axis=-1)

            all_scores_for_bce = tf.concat([pos_scores, neg_scores], axis=0)

            labels_for_bce = tf.concat([tf.ones_like(pos_scores), tf.zeros_like(neg_scores)], axis=0)

            # Main Link Prediction (BCE)
            L_link_prediction = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=labels_for_bce, logits=all_scores_for_bce))

            reg_loss = hparams['reg_lambda'] * (
                tf.nn.l2_loss(pos_user_embs) + 
                tf.nn.l2_loss(pos_item_embs) + 
                tf.nn.l2_loss(neg_item_embs)
            ) / tf.cast(tf.shape(pos_user_embs)[0], tf.float32)
            L_link_prediction += reg_loss
            ...
            # Main Loss
            total_loss = (tf.cast(hparams['w_link_pred'] * L_link_prediction, tf.float32) +
        ...
        return total_loss, L_link_prediction, L_cl, L_kl_gen, L_cl_model_loss
    ...
    for epoch in range(1, hparams['epochs'] + 1):
        ...
        for i in range(num_batches):
            ...
            total_loss, L_link_prediction, L_cl, L_kl_gen, L_cl_model_loss = train_step(
            ...
            epoch_losses['link_prediction'] += L_link_prediction
        ...
        # Print current epsilon values for analysis (final state after epoch)
        ...
        dev_results, per_user_metrics = evaluate_model(
            gnn_model, train_df, user_map, item_map, test_df,
            reverse_user_map, reverse_item_map, num_users, num_items, train_adj_coo
        )
        eval_end_time = time.time()
        print(f"Evaluation Duration: {(eval_end_time - eval_start_time):.2f} s")

        current_auc_roc = dev_results.get('auc_roc', 0.0)
        if current_auc_roc > best_auc_roc_so_far:
            print(f"New best AUC_ROC score: {current_auc_roc:.6f}. Saving model weights.")
            best_auc_roc_so_far = current_auc_roc
            gnn_model.save_weights("best_main_model.weights.h5")
            epsilon_generator.save_weights("best_epsilon_generator.weights.h5")

        print(f"Best AUC_ROC so far: {best_auc_roc_so_far:.6f}")
...
if __name__ == "__main__":
    sys.stdout = FileLogger("model_log_link_prediction_pipeline.txt")
    ...
    # Data Loading
    train_df = load_interactions('./data/wiki40/wiki40_train.txt', has_prefix=False)
    test_df  = load_interactions('./data/wiki40/wiki40_test.txt', has_prefix=False)
    user_map, item_map = build_mappings_from_train(train_df)
    reverse_user_map = {idx: u for u, idx in user_map.items()}
    reverse_item_map = {idx: i for idx, i in item_map.items()} 

    num_users, num_items = len(user_map), len(item_map)
    
    positive_train_df = train_df[train_df['label'] == 1]
    train_adj_coo = coo_matrix((np.ones(len(positive_train_df), dtype=np.float32), 
                                                (positive_train_df['user'].map(user_map).values, positive_train_df['item'].map(item_map).values)),
    ...
    train_model(joint_adj_tf, train_df, test_df, gnn_model, epsilon_generator, HPARAMS, reverse_user_map, reverse_item_map, num_users, num_items, user_map, item_map)
